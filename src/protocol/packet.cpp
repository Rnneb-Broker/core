...
ConnectPayload ConnectPayload::deserialize(const uint8_t* data, size_t len) {
    ...
    if (!reader.empty()) payload.username = reader.read_string();
    if (!reader.empty()) payload.password = reader.read_string();
    if (reader.remaining() >= 2) payload.keepalive = reader.read_u16();
    return payload;
}
