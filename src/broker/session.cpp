...
void Session::handle_connect() {
    ...
    try {
        auto payload = ConnectPayload::deserialize(
...
void Session::handle_subscribe() {
    ...
    try {
        auto payload = SubscribePayload::deserialize(
            payload_buffer_.data(), payload_buffer_.size());
...
void Session::send_suback(uint16_t packet_id, const std::vector<QoS>& granted_qos) {
    BinaryWriter writer;
