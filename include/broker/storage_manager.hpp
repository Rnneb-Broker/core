...
static_assert(sizeof(MessageHeader) == 16,
              "MessageHeader must be 16 bytes (packed)");
...
private:
  static constexpr uint64_t INDEX_INTERVAL =
      1024; // One entry per 1024 messages
...
public:
  ...
  uint64_t get_last_offset() const {
    return next_offset_.load(std::memory_order_acquire) - 1;
  }
...
public:
  ...
  using FailureCallback = std::function<void(
      const std::string &topic, int error_code, uint64_t segment_id)>;

  explicit FlushWorker(SegmentLog *log,
                       DurabilityMode mode = DurabilityMode::FAST);
  ~FlushWorker();
...
private:
  ...
  void handle_flush_error(const std::string &topic, int error_code,
                          uint64_t segment_id);
};
...
public:
  struct Config {
    ...
    FlushWorker::DurabilityMode durability_mode =
        FlushWorker::DurabilityMode::FAST;

    Config() = default;
  };

  explicit StorageManager(Broker *broker, Config config);
  ~StorageManager();
   ...
   */
  void on_storage_failure(const std::string &topic, int error_code,
                          uint64_t segment_id);
  ...
  get_topic_stats(const std::string &topic) const;
